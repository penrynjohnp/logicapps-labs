...
description: Learn how to connect a conversational Logic Apps agent to external services using connectors and expose connector actions as tools.
...
In this module, you will learn how to connect your conversational Logic Apps agent to external systems using connectors, and how to expose connector operations to the agent as tools it can call during a conversation.
...
Helpful resources:
- Logic Apps connectors overview: https://learn.microsoft.com/en-us/connectors/overview

---

## Why connectors add value for conversational agents

Logic Apps connectors provide a fast, secure, and reliable way to let an agent take action in the real world without custom integration code.
...
- Secure connections: Standardize authentication with connections (OAuth, API keys, managed identity). Store secrets securely and reuse connections across tools.
...
- Connectors
	- Reusable integrations that let Logic Apps call external services (Microsoft 365, GitHub, Azure services, SaaS apps, and more).
...
- Tools (in a conversational agent)
	...
	- In Logic Apps agents, a tool can be backed by a connector action, a built-in action, or another workflow.
...
> [!NOTE]
> Connector triggers are not supported in conversational agents. Conversational agents start when a chat session is initiated (for example, from an integrated chat client). There is no separate workflow trigger inside the agent; tool actions are invoked by the agent during the conversation.
---
...
- For the exercises:
	...
	- Optional authenticated example: a GitHub account and either OAuth or a PAT with minimal read scopes.

> [!NOTE]
> This module focuses on connector-backed tools. Agent parameters and On-Behalf-Of (OBO) patterns come later.
...
- Connections and authentication: OAuth, API key, managed identity, or none depending on the connector.
- Inputs and outputs: actions define parameters and return JSON; your agent summarizes outputs for the user.
- Limits and reliability: expect throttling (429), timeouts, retries, and pagination; design prompts and tools to handle gracefully.
...
- Feed URL: set a fixed URL (no parameters yet), e.g.
...
### Step 5 — Test in chat
- Ask: “What is new from the Logic Apps blog?”
...
- Connection: no auth required; create with defaults if prompted.
- Configure with fixed inputs (no parameters yet), e.g., Location = “Seattle, US”, Units = “imperial”.
...
- Authenticate with OAuth (least-privilege scopes) or a PAT with minimal read access.
- Prefer secure secret storage (e.g., Key Vault-backed references) where available.
...
- Map static example inputs directly on the action (no agent parameters yet), e.g., owner=“octocat”, repo=“Hello-World”, number=“1”.
- Save.
...
- Provide examples and hints: improve parameter extraction with formats (e.g., owner/repo) and sample utterances.
...
- Any REST API: use HTTP built-in, or managed connector if available.
...
- If you created new connections (e.g., GitHub), remove or rotate credentials per your org’s policy.
